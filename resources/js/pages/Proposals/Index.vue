...
function convertRecord(record: Record<string, any>) {
    router.post(`${props.endpoints.convert}/${record.id}/converter`, {}, { preserveScroll: true });
}
</script>
...
<template>
    ...
    <div class="space-y-6 px-4 py-6 md:px-6">
        ...
        >
            <template #actions>
                <Button type="button" variant="secondary" @click="resetForm">Nova proposta</Button>
            </template>
        </PageIntro>
                            ...
                            <tr v-for="record in records" :key="String(record.id)" class="border-b border-border/60 last:border-none">
                                ...
                                <td class="py-4">
                                    <div class="flex flex-wrap gap-2">
                                        ...
                                        <Button type="button" size="sm" variant="secondary" @click="convertRecord(record)">Converter</Button>
            ...
            <article class="rounded-[2rem] border border-border/80 bg-card/95 p-6 shadow-[0_16px_40px_rgba(60,43,30,0.08)]">
                ...
                <div class="mt-4 grid gap-4 md:grid-cols-2">
                    ...
                    <div class="rounded-[1.5rem] bg-secondary/45 px-4 py-3 text-sm">
                        <p class="font-medium text-foreground">Totais calculados</p>
                        <p class="mt-2 text-muted-foreground">
                            Subtotal € {{ totals.subtotal.toFixed(2) }} · IVA € {{ totals.tax.toFixed(2) }} · Total € {{ totals.total.toFixed(2) }}
                        </p>
                    </div>
                </div>
                ...
                <div class="mt-6 flex gap-3">
                    <Button type="button" @click="submit">{{ editingId ? 'Guardar alterações' : 'Criar proposta' }}</Button>
